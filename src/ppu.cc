...
PPUTimer::PPUTimer(PPU& ppu, std::mutex& lineMutex,
...
PPU::PPU(sf::RenderWindow& window) : window(window), regs(PPURegs(*this)) {}

CharacterROM::CharacterROM() {
    std::ifstream ifile;
    ifile.open("/Users/steve/code/mario++/misc/smb1-us.nes",
...
void PPU::draw() {
    ...
    sf::Image bimg {{512, 240}, (byte*)bkgd};
    sf::Texture btex {sf::Vector2u {256, 240}};
    assert(btex.loadFromImage(bimg, false,
                              sf::IntRect({regs.scroll >> 8, 0}, {256, 240})));
    sf::Sprite bspr {btex};
    ...
    for (int i = 0; i < 64; i++) {
        ...
        if (s.y < 0xef) {
            ...
            // s.x, s.y, s.index, s.attrs);
            sprites[i].draw((Pixel*)fore, *this);
        }
    }

    sf::Image simg {{256, 240}, (byte*)fore};
    sf::Texture stex {sf::Vector2u {256, 240}};
    assert(stex.loadFromImage(simg, false, sf::IntRect({0, 0}, {256, 240})));
    sf::Sprite sspr {stex};
    window.draw(sspr);
}
...
void NameTable::draw(Pixel* pixels, unsigned x, unsigned y, PPU& ppu) {
    for (int j = 0; j < 30; j++) {
        for (int i = 0; i < 32; i++) {
            ...
            tiles[j][i].draw(pixels, x + i * 8, y + j * 8,
                             ppu.palettes.bkgdPalettes[pid], ppu);
        }
    }
}
...
void Pattern::draw(Pixel* pixels, unsigned w, unsigned x, unsigned y,
                   Palette& pal, bool xflip, bool yflip) {
    for (int j = 0; j < 8; j++) {
        byte b1 = data[j + 0];
        byte b2 = data[j + 8];
        for (int i = 0; i < 8; i++) {
    ...
    switch (index) {
        ...
        case 5:
            ...
            if (!gotFirstByte) {
                ...
                gotFirstByte = false;
                assert(!(value &
                         0xff));  // not expecting to deal with Y-scrolling
            }
            return;
        ...
        case 7: {
            ...
            if (ppuAddr >= 0x2000 && ppuAddr < 0x3000) {
                ...
                if (addr == 0x10 || addr == 0x14 || addr == 0x18 ||
                    addr == 0x1c) {
                    // from above url:
